...
const ChatStream = () => {
    ...
    const startChat = async (initialQuestion) => {
        ...
        try {
            // Create a mock request object for the chat_route
            const req = {
                json: async () => ({ question: initialQuestion }), // Mock request body
            };
    
            // Call the chat_route directly
            const response = await chat_route(req); // Assuming chat_route returns a response object
